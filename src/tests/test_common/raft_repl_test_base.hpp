...
public:
    ...
    /*ReplaceMemberStatus check_replace_member_status(std::shared_ptr< TestReplicatedDB > db, uuid_t task_id,
                                                    replica_id_t member_out, replica_id_t member_in) {
        ReplaceMemberStatus status = ReplaceMemberStatus::UNKNOWN;
        this->run_on_leader(db, [this, db, task_id, member_out, member_in, &status]() {
            LOGINFO("check replace member status, task_id={}, out={} in={}", boost::uuids::to_string(task_id),
                    boost::uuids::to_string(member_out), boost::uuids::to_string(member_in));
            ...
            auto ret =
                hs()->repl_service().get_replace_member_status(db->repl_dev()->group_id(), task_id, out, in, others);
            LOGINFO("check replace member status, status={}", ret)
            status = ret;
        });
        LOGINFO("check replace member status, status={}", status)
        return status;
    }*/
